...
For a pipeline owned by one team and expressed as one program — call Textract, classify, wait for review, then store — durable functions keep the execution machinery small.

For a pipeline organized around independently owned services, the [EventBridge ownership model](document-pipeline-eventbridge.md) is a better fit. Each accepted handoff records who is responsible next, and services can evolve or add subscribers without changing a central workflow. Ownership, overdue work, and recovery are explicit application state. The tradeoff is maintaining the delivery and consistency machinery that durable execution supplies. Durable functions remain useful inside an individual stage.
