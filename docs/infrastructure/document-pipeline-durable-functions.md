...
**Model it as an append-only event log, not a row per document.** Instead of every service racing to update the same shared item (`document_id` → `status`), have each one write a new, immutable item — `document_id` as the partition key, something like `timestamp#stageName` as the sort key. There's no write contention to reason about, because nobody is ever overwriting anybody else's write; a slow, late-arriving update from one service just becomes an earlier event in the log rather than a race against a newer one on the same attribute. It also gives you a real audit trail for free — "what happened to this document, in order" — which the Lambda-native execution history doesn't, since that's subject to its own retention window.
...
One thing an append-only log doesn't give you cheaply on its own: "show me every document currently in `pending-review`." Answering that means knowing, for each document, that no *later* event exists — which an event log alone doesn't answer without scanning. The fix is for that same recording Lambda to also maintain a small current-state projection alongside the log — one item per document with `currentStage` and `updatedAt`, backed by a GSI on `currentStage` — updated as a side effect of appending each event. You end up with both: the log for history and debugging, the projection for the dashboard query, and one writer that keeps them consistent.

And the same distinction from earlier still holds here: recording an event is not the same thing as resuming the orchestrator. A service that finishes a stage the orchestrator is actually blocked on needs to do both — call the recording API (so the log and dashboard reflect it) and resolve the pipeline's `wait_for_callback` (so the pipeline actually continues) — but the recording call by itself never advances anyone's workflow.
